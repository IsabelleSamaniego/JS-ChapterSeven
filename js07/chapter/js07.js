...
// run an anonymous function in response to the change event
document.getElementById("getFile").onchange = function() {
    
    // Retrieve information about the selected file
    let userFile = this.files[0];
    // userFile -- file object selected by the user
    // files -- files collection contains the files selected by the user
    ...
    // CREATE A FileReader OBJECT TO LOAD THE CONTENTS OF UserFile OBJECT
    // Read the contents of the selected file
    let fr = new FileReader();
    fr.readAsText(userFile);

    // APPLY THE ONLOAD EVENT
